...
NoSubj(INT), mark(INT 0-100)

Output 

mean(DOUBLE) Grade(String:A-B-C-D-E-F)
...
validation: allow input if mark<100 && mark>0
--> store totalMarks=totalMarks+mark, 
compute mean=totalMarks/totalSub
```
...
i(turns remaining)
```
Bonus: I use the for loop and if else for the turns, if the user gets the correct answer, we stop the process using `break`.
